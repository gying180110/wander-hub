...
INSERT INTO t_announcement (title, content, active, play_seconds, sort_no)
SELECT '欢迎使用 Wander', '系统已支持角色权限、公告轮播、改动记录。', 1, 6, 10
WHERE NOT EXISTS (SELECT 1 FROM t_announcement WHERE title = '欢迎使用 Wander');

INSERT INTO t_change_log (version_no, module_name, change_point, change_file)
SELECT 'v1.1.0', 'wander-hub-auth', '新增角色和权限字段', 'wander-hub-app/src/main/resources/schema.sql'
